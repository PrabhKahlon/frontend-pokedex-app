...
export default function PokemonCard({ name, url }: Pokemon) {
    ...
    return (
        <div className={`transition-all duration-1000 md:mx-4 w-full bg-no-repeat bg-right flex align-middle justify-center items-center h-40 ${bgColour} my-4 rounded-xl max-w-md ` + (isLoading ? "animate-pulse" : "")} onClick={isLoading ? () => {} : handleClick}>
            {isLoading ? <CardLoading /> :
                <>
                    ...
                    <div className="bg-no-repeat flex w-32">
                        <img className="animate-fade-in h-32 w-32 translate-x-16 z-10" src={data?.sprites["front_default"]} alt={`${name} front sprite`}></img>
                    </div>
                    <div className="bg-no-repeat flex flex-1 h-full w-10 opacity-50" style={{ WebkitMask: "url('/images/pokeball-outline.svg')", WebkitMaskSize: "cover", backgroundColor: "white" }}>
                    </div>
                </>
